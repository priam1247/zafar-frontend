document.addEventListener('DOMContentLoaded', function () {
  ...
  if (greetingEl) {
    ...
    // "Student" is only the fallback for accounts that never got a name
    // saved (pre-dates the name field, or it's blank) — everyone else
    // gets greeted by their actual first name once /auth/me resolves.
    if (textNode) textNode.textContent = 'Good ' + part + ', Student';
    Zafar.authGet('/auth/me')
      .then(function (user) {
        if (textNode && user && user.name) {
          var firstName = user.name.trim().split(/\s+/)[0];
          textNode.textContent = 'Good ' + part + ', ' + firstName;
        }
      })
      .catch(function () {}); // keep the "Student" fallback already shown
  }
